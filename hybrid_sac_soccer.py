...
from wrappers.soccer_wrapper import SoccerScaledParameterisedActionWrapper, SoccerParameterisedActionWrapper
import gym_soccer
import hfo_py
...
class Policy(nn.Module):
    ...
    def get_action(self, x, device):
        ...
        log_prob_c = torch.cat([all_log_prob_c[:, :2].sum(1, keepdim=True), all_log_prob_c[:, 2].unsqueeze(-1), all_log_prob_c[:, 3:].sum(1, keepdim=True)], 1)
...
if args.autotune:
    ...
    a_d_optimizer = optim.Adam([log_alpha_d], lr=args.q_lr)
else:
    alpha = args.alpha
    alpha_d = args.alpha
...
for global_step in range(1, args.total_timesteps + 1):
    ...
    next_obs, reward, done, _ = env.step(action)  # because of wierd env stuff
    next_obs *= 1.0
    ...
    if len(rb.buffer) > args.batch_size:  # starts update as soon as there is enough data.
        ...
        qf_loss.backward()
        nn.utils.clip_grad_norm_(list(qf1.parameters()) + list(qf2.parameters()), 0.5)
    ...
    if len(rb.buffer) > args.batch_size and global_step % 100 == 0:
        ...
        writer.add_histogram("actions_c/dash_p", action[1], global_step)
        writer.add_histogram("actions_c/dash_d", action[2], global_step)
        writer.add_histogram("actions_c/turn_a", action[3], global_step)
        writer.add_histogram("actions_c/kick_p", action[4], global_step)
        writer.add_histogram("actions_c/kick_d", action[5], global_step)
        ...
        writer.add_scalar("debug/max_r", s_rewards.max(), global_step)
        writer.add_scalar("debug/min_r", s_rewards.min(), global_step)
        # writer.add_scalar("debug/ent_bonus", (alpha * ent_c + alpha_d * ent_d).mean().item(), global_step)
